...
Client.prototype.addSocket = function(socket) {
  ...
  if (
    Object.keys(this.sockets).length == 1 // it's the first guy in, useful for mobile where there's just one guy and no plans to call activate
  )
...
Client.prototype.handleNewSocket = function(socket) {
  ...
  if (!this.isAuthorized && this.activeSocketId != socket.id) {
    console.log(
      "Info: Activating this socket since no socket on client is yet authorized: ",
      socket.id
    );
...
Client.prototype.activate = function(socketId) {
  ...
  if (this.activeSocketId == socketId) {
    console.log(
      "Error: Trying to activate an already active socket: ",
      socketId
    );
    return;
  }
  ...
  this.socList().forEach(s => {
    if (s.id != socketId) {
      console.log(
        "Info: Emitting otherActiveSession and disconnecting socket: ",
        s.id
      );
